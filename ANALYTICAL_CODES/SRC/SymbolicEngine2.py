...
def get_schwarzschild_spherical():
    ...
    # Pack all required variables into a standard dictionary
    return {
...
def get_reissner_nordstrom_isotropic():
    ...
    g_inv = g_cov.inv()
    sqrt_det_g = sp.simplify(alpha_func * H**3)
...
def get_flrw_cartesian():
    ...
    t, x, y, z = sp.symbols('t x y z', real=True)
    # Scale factor defined as an arbitrary function of time
    a_t = sp.Function('a')(t) 
...
def get_bardeen_spherical():
    ...
    t, r, theta, phi = sp.symbols('t r theta phi', real=True)
    M, Qm = sp.symbols('M Qm', real=True, positive=True) # Qm is the magnetic monopole charge
    
    # The regularized metric function
...
def get_kerr_boyer_lindquist():
    ...
    # ADM Variables
    alpha_func = sp.sqrt(Sigma * Delta / A_term)
    ...
    g_cov = sp.Matrix([
        [-(alpha_func**2) + gamma_pp * beta_phi**2, 0, 0, gamma_pp * beta_phi],
        [0, gamma_rr, 0, 0],
        [0, 0, gamma_ttheta, 0],
    ...
    # Clean the branch cut for the volume element
    sqrt_det_g = sp.simplify(alpha_func * sp.sqrt(gamma_rr * gamma_ttheta * gamma_pp)).replace(sp.Abs(sp.sin(theta)), sp.sin(theta))

    # Eulerian Observer Tetrad (ZAMO)
    Tetrad = sp.Matrix([
        [alpha_func, 0, 0, 0],
...
def get_kerr_quasi_isotropic_spherical():
    ...
    # Quasi-isotropic radial mapping
...
def get_kerr_schild_cartesian():
    ...
    # Implicit definition of r solved for r^2
    r2 = (R2 - a**2)/2 + sp.sqrt((R2 - a**2)**2 / 4 + a**2 * z**2)
    r_val = sp.sqrt(r2)

    # Scalar perturbation function
    ...
    # Build the Eulerian Observer Tetrad (ZAMO) for Kerr-Schild
    Tetrad = sp.zeros(4, 4)
    Tetrad[0, 0] = alpha_func

    for k in range(3):
        # Time components of spatial legs (incorporating the shift vector)
        Tetrad[k+1, 0] = (f / sp.sqrt(1 + f)) * l_vec[k]
        for i in range(3):
            # Spatial components of spatial legs
            delta_ki = 1 if k == i else 0
            Tetrad[k+1, i+1] = delta_ki + (sp.sqrt(1 + f) - 1) * l_vec[k] * l_vec[i]

    # Build the full covariant metric g_uv = eta_uv + f * l_u * l_v
    g_cov = sp.zeros(4, 4)
    l_mu = [1, l_x, l_y, l_z]
    eta = sp.diag(-1, 1, 1, 1)
    ...
    # MASSIVE OPTIMIZATION: The determinant of the KS metric is exactly -1 everywhere!
