...
def build_tetrads(data: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    ...
    # hat{3} - Rigorous Normalization (Fixes Eq A18 typo)
    raw_3 = {}
    for comp in ["t", "x", "y", "z"]:
...
def gem_fields(theta, F):
    """
    Requested by User: Absolute definition without filtering.
    Calculates E = n^t * F_it
    """
    alp = theta["0_t"]
...
# --------------------------- Main --------------------------- #
def main(data_path: str = DATA_FILE, out_png: str = "Paper_Method_NoFilter.png"):
    ...
    # Use the specific user-requested definition
