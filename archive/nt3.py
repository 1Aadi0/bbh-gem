...
def get_full_binary_grid(data):
    """
    Reconstructs the full binary grid with proper symmetries.
    """
    ...
    # Mirror/Rotate
    # Scalars (Lapse, Metric): 180 deg rotation = Flip X + Flip Y
    ...
    # Vectors (Shift): 180 deg rotation flips direction -> Flip X + Flip Y + Sign Change
...
def build_tetrads(data: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    ...
    # Hat 3: Rigorous Normalization (Fixes Mathematical Error)
    raw_3 = {}
    for comp in ["t", "x", "y", "z"]:
...
# --------------------------- Main Plotting --------------------------- #
def main(data_path: str = DATA_FILE, out_png: str = "Paper_Method_Tetrads3.png"):
    ...
    # Prepare plotting arrays with correct orientation
    if data['alp'].shape == (len(y), len(x)):
        vals_alp = data['alp']
        vals_bz = Bz_phys
        vals_Ex = E["0"][0]
        vals_Ey = E["0"][1]
        vals_bx = data['betax'] # Shift Vector for streamlines
        vals_by = data['betay']
    else:
        vals_alp = data['alp'].T
        vals_bz = Bz_phys.T
        vals_Ex = E["0"][0].T
        vals_Ey = E["0"][1].T
        vals_bx = data['betax'].T
        vals_by = data['betay'].T
    ...
    # --- Plot 2: Magnetic Field (Improved) ---
    # Background: Magnetic Magnitude (Curl of Shift)
    ...
    # Lines: Shift Vector Streamlines (Frame Dragging Swirls)
    # This visualizes the "Toroidal" field lines similar to the paper
    ...
    axes[1].set_title("Gravito-Magnetic Field (Frame Dragging)")
