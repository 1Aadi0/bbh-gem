...
# --------------------------- Main Plotting --------------------------- #
def main(data_path: str = DATA_FILE, out_png: str = "Comparison_Quad_Plot1.png"):
    ...
    # MASKING: Create a "Masked" Shift Vector
    # We hide the shift vector where Lapse < 0.3 (inside the horizons)
    # This stops lines from drawing "inside" or "between" the holes deeply
    mask_indices = vals_alp < 0.4  # Adjust 0.4 to change the "hole" size
    vals_betax_masked = vals_betax.copy()
    vals_betay_masked = vals_betay.copy()
    vals_betax_masked[mask_indices] = np.nan
    vals_betay_masked[mask_indices] = np.nan
    ...
    ax.set_title("1. Electric Field Lines (Dipole)")
    ax.set_aspect("equal")
    
    # Plot 2: Magnetic Field Lines (Eq 8)
    ax = axes[0, 1]
    ...
    ax.set_title("2. Spatial Tetrad B^3 (Saddle)")
    ax.set_aspect("equal")

    # Plot 3: Shift Streamlines (PAPER REPLICA)
    # Now using MASKED shift vector to avoid the "elliptical path inside"
    ax = axes[1, 0]
    # Background: Lapse (Red) - Matches Paper Figure 3 Top Row
    ax.pcolormesh(vals_x, vals_y, vals_alp, cmap="Reds", shading="auto", vmin=0.0, vmax=1.0)
    # Lines: Masked Shift Vector
    ax.streamplot(vals_x, vals_y, vals_betax_masked, vals_betay_masked, color="black", density=1.2, arrowsize=1.0)
    ax.contour(vals_x, vals_y, vals_alp, levels=[0.3], colors="k", linewidths=0.8) # Horizon outline
    ax.set_title("3. Shift Streamlines (Masked Horizons)")
    ax.set_aspect("equal")
    
    # Plot 4: Shift Magnitude (Check)
    ax = axes[1, 1]
    ax.pcolormesh(vals_x, vals_y, vals_Shift_mag, cmap="viridis", shading="auto")
    ax.streamplot(vals_x, vals_y, vals_betax_masked, vals_betay_masked, color="white", density=1.2, arrowsize=1.0)
    ax.set_title("4. Shift Magnitude & Masked Lines")
    ax.set_aspect("equal")
