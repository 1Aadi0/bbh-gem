...
• **HDF5 ingestion** of lapse, shift, and spatial metric  
• **Synchronization** of variables across inconsistent iteration numbers  
• **Extraction** of equatorial spacetime slices  
• **Reconstruction** of the full binary via rotational symmetry ($180^\circ$ around Z)  
• **Construction** of rigorous orthonormal tetrads on the numerical grid  
• **Computation** of Maxwell-like field tensors (E_ij, B_ij)  
• **Visualization** of field lines and frame-dragging vortices  
...
.
├── src/
│   ├── load_etk_data.py           # Low-level HDF5 I/O and iteration matching for ETK output
│   ├── nr_snapshot_builder.py     # Builds synchronized spacetime slices from raw data
│   ├── binary_symmetry.py         # Reconstructs the full BBH domain via 180° rotation
│   └── numerical_gem_pipeline.py  # Core GEM engine: Tetrad construction, Field extraction, Plotting
│
├── results/                       # Output directory for analysis products
│   └── [Run_Name]/                # Subdirectory auto-generated for each simulation
│       ├── figures/               # Final visualizations (E/B fields, Tetrad checks)
│       └── par/                   # Parameter files preserving run configuration
│
├── runs/                          # Local storage for raw Einstein Toolkit data (Git-ignored)
├── archive/                       # Legacy development scripts and experiments
...
It performs:
    ...
    • Discovery of all available iterations and refinement levels
    • Loose matching between lapse, shift, and metric files
    • Selection of a common refinement level
    • Identification of a consistent reference iteration
...
This step constructs a physically meaningful spacetime slice by:
    ...
    • Extracting a single equatorial spatial plane
    • Aligning lapse, shift, and metric on the same iteration
    • Converting Carpet AMR blocks into a uniform Cartesian grid
    • Producing a consistent ADM dataset
...
with the full two–black–hole geometry.
...
This step:
    ...
    • Builds numerical orthonormal tetrads from the ADM metric
    • Enforces orthonormality using the full inverse 4-metric
    • Computes the DGREM field tensor
    • Extracts gravito-electric field E (tidal curvature)
    • Extracts gravito-magnetic field B (frame-dragging curvature)
    • Generates publication-quality plots

The output includes:

    • Field-line visualizations
    • Color maps of E and B
    • Binary horizon contours
