...
.
├── src/
│   ├── load_etk_data.py           # Low-level HDF5 I/O and iteration matching for Einstein Toolkit output
│   ├── nr_snapshot_builder.py     # Builds a synchronized spacetime slice from ETK data
│   ├── binary_symmetry.py         # Reconstructs the full BBH domain using 180° rotational symmetry
│   └── numerical_gem_pipeline.py  # Core GEM engine: Ghost zones resolution, tetrads, E/B fields, and plotting
│
├── archive/                       # Older development versions and experimental scripts
├── output-0000-active/            # Example Einstein Toolkit output (not required to run the pipeline)
├── final_data.npz                 # Extracted spacetime snapshot (generated by nr_snapshot_builder.py)
├── README.md
└── LICENSE
